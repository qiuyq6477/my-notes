...
<node TEXT="顶点数组对象（也称为 VAO）可以像VBO一样进行绑定，并且从该点开始的任何后续顶点属性调用都将存储在 VAO 中。" ID="ID_509876362" CREATED="1752399149728" MODIFIED="1752399170803"/>
<node TEXT="所以在配置顶点属性指针时，只需要进行一次这些调用，每当需要绘制对象时，只需绑定相应的 VAO即可。这使得在不同顶点数据和属性配置之间切换就像绑定不同的 VAO 一样简单。" ID="ID_240930045" CREATED="1752399171912" MODIFIED="1752399198523"/>
<node TEXT="VAO存储以下内容" POSITION="bottom_or_right" ID="ID_1779758077" CREATED="1752399212078" MODIFIED="1752399224373">
<node TEXT="调用 glEnableVertexAttribArray 或 glDisableVertexAttribArray。" ID="ID_1366908940" CREATED="1752399230965" MODIFIED="1752399236725" MAX_WIDTH="20 cm"/>
<node TEXT="通过 glVertexAttribPointer 进行顶点属性配置。" ID="ID_1447197753" CREATED="1752399240545" MODIFIED="1752399240912"/>
<node TEXT="VBO" ID="ID_1777887243" CREATED="1752399245076" MODIFIED="1752399458819" MAX_WIDTH="20 cm"/>
<node TEXT="VAO" ID="ID_1502392248" CREATED="1752399459064" MODIFIED="1752399461506"/>
</node>
</node>
<node TEXT="glBindVertexArray" POSITION="bottom_or_right" ID="ID_1874880955" CREATED="1751074754359" MODIFIED="1751092768947">
...
<node TEXT="glGenBuffers" POSITION="bottom_or_right" ID="ID_516062509" CREATED="1751091781234" MODIFIED="1751091782414">
...
<node TEXT="作用：生成缓冲区用于存储顶点数据 (如位置、颜色、法线等)" ID="ID_1169274740" CREATED="1751091812992" MODIFIED="1752397258536"/>
</node>
<node TEXT="glBindBuffer" POSITION="bottom_or_right" ID="ID_329153367" CREATED="1751091846777" MODIFIED="1751091847972">
...
<node TEXT="作用：将缓冲区绑定到指定目标，可以同时绑定多个不同目标类型的缓冲区，后续的操作只会影响当前绑定的缓冲区" ID="ID_1700694094" CREATED="1751092707773" MODIFIED="1752397214544"/>
</node>
<node TEXT="glBuffData" POSITION="bottom_or_right" ID="ID_1647500602" CREATED="1751091911588" MODIFIED="1751091941711">
...
<node TEXT="GL_STATIC_DRAW：数据不会或很少改变" ID="ID_291780857" CREATED="1751092062219" MODIFIED="1752397428013"/>
<node TEXT="GL_DYNAMIC_DRAW：数据会被频繁改变" ID="ID_1362335740" CREATED="1751092062220" MODIFIED="1752397430243"/>
<node TEXT="GL_STREAM_DRAW：数据每次绘制时都会改变" ID="ID_717345991" CREATED="1751092062220" MODIFIED="1752397432655"/>
</node>
<node TEXT="作用：将用户定义的数据复制到当前绑定缓冲区。" ID="ID_1951896589" CREATED="1751092734607" MODIFIED="1752397307523"/>
</node>
</node>
<node TEXT="配置缓冲区" ID="ID_181133776" CREATED="1752398809893" MODIFIED="1752398816063">
<node TEXT="glVertexAttribPointer" POSITION="bottom_or_right" ID="ID_1941435266" CREATED="1751092188313" MODIFIED="1751092189295">
...
<node TEXT="可以指定为 0 让 OpenGL 确定来确定是哪个值，这仅在值紧密打包时有效" ID="ID_716038655" CREATED="1752398960437" MODIFIED="1752399015483"/>
</node>
<node TEXT="pointer：当前缓冲中该属性的起始偏移量" ID="ID_612410887" CREATED="1751092248543" MODIFIED="1751092248543"/>
<node TEXT="作用：指定顶点属性的格式和位置。手动指定输入数据的哪一部分转到顶点着色器中的哪个顶点属性" ID="ID_1429643571" CREATED="1751092860269" MODIFIED="1752398888446"/>
<node TEXT="每个顶点属性都从 VBO 管理的内存中获取其数据，并且它从哪个 VBO 获取数据（可以有多个 VBO）由调用 glVertexAttribPointer 时当前绑定到 GL_ARRAY_BUFFER 的 VBO 确定" ID="ID_1628118203" CREATED="1752399063926" MODIFIED="1752399064396"/>
</node>
<node TEXT="glEnableVertexAttribArray" POSITION="bottom_or_right" ID="ID_994737720" CREATED="1751092259063" MODIFIED="1751092261271">
...
<node TEXT="删除缓冲区" ID="ID_1790643340" CREATED="1752398821318" MODIFIED="1752398826653">
<node TEXT="glDeleteBuffers" POSITION="bottom_or_right" ID="ID_773528651" CREATED="1751169927082" MODIFIED="1751169935254"/>
...
<node TEXT="glDrawElements" POSITION="bottom_or_right" ID="ID_154449416" CREATED="1752399602339" MODIFIED="1752399733411">
<node TEXT="mode" ID="ID_1586259270" CREATED="1751073834191" MODIFIED="1751073843649">
<node TEXT="GL_TRIANGLES" ID="ID_1054045143" CREATED="1751073845176" MODIFIED="1751073851692"/>
</node>
<node TEXT="count" ID="ID_739850288" CREATED="1751073859193" MODIFIED="1752399723300">
<node TEXT="指定要渲染的索引数量" ID="ID_1799301403" CREATED="1751073878114" MODIFIED="1752399724332"/>
</node>
<node TEXT="type" ID="ID_1284437986" CREATED="1751073864784" MODIFIED="1752399737298">
<node TEXT="指定索引数据的类型" ID="ID_825429073" CREATED="1751073867691" MODIFIED="1752399728877"/>
</node>
<node TEXT="indices" ID="ID_1163579553" CREATED="1752399738329" MODIFIED="1752399738766">
<node TEXT="索引数组的位置指针，取决于是否使用了元素缓冲区对象（EBO）" ID="ID_31216382" CREATED="1752399748016" MODIFIED="1752399827291" MAX_WIDTH="20 cm"/>
<node TEXT="使用 EBO 时：&#xa;传递 NULL 或偏移量（如 (GLvoid*)(2 * sizeof(GLuint))），表示从当前绑定的 EBO 中读取索引。" POSITION="bottom_or_right" ID="ID_923505893" CREATED="1752399816096" MODIFIED="1752399817365"/>
<node TEXT="不使用 EBO 时：&#xa;直接传递指向 CPU 内存中索引数组的指针（不推荐，性能较差）。" POSITION="bottom_or_right" ID="ID_531916603" CREATED="1752399821535" MODIFIED="1752399821840"/>
</node>
<node TEXT="作用" ID="ID_606702587" CREATED="1752399683912" MODIFIED="1752399689165">
<node TEXT="允许通过索引多次引用同一顶点，适合复杂模型（如共享顶点的网格）" ID="ID_1811170647" CREATED="1752399689309" MODIFIED="1752399690038"/>
<node TEXT="相比 glDrawArrays，减少了顶点数据的重复存储，节省内存带宽" ID="ID_976643767" CREATED="1752399695330" MODIFIED="1752399695572"/>
<node TEXT="通过不同的索引顺序，可以用同一组顶点数据渲染不同的图元组合" ID="ID_966714777" CREATED="1752399701796" MODIFIED="1752399702044"/>
</node>
</node>
...
<node TEXT="从CPU向显卡发送数据相对较慢，所以只要有可能，我们就尽量一次性发送尽可能多的数据。" ID="ID_1301416933" CREATED="1752396041402" MODIFIED="1752396042554"/>
...
<node TEXT="glBindBuffer：绑定 VBO 到指定目标（比如GL_ARRAY_BUFFER）。" ID="ID_1564087282" CREATED="1751093430244" MODIFIED="1752396824268" MAX_WIDTH="20 cm"/>
<node TEXT="glBufferData：向 VBO 中填充数据。" ID="ID_992643954" CREATED="1751093430245" MODIFIED="1751093430245"/>
</node>
<node TEXT="使用方法" ID="ID_97093470" CREATED="1751095558457" MODIFIED="1751095563488">
<node TEXT="// 1. 创建 VBO&#xa;GLuint VBO;&#xa;glGenBuffers(1, &amp;VBO);" ID="ID_1182448437" CREATED="1752396851034" MODIFIED="1752396852542"/>
<node TEXT="// 2. 绑定 VBO 到 GL_ARRAY_BUFFER 目标&#xa;glBindBuffer(GL_ARRAY_BUFFER, VBO);" ID="ID_299461081" CREATED="1752396856276" MODIFIED="1752396857310"/>
<node TEXT="// 3. 向 VBO 中填充数据&#xa;glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);" POSITION="bottom_or_right" ID="ID_904948841" CREATED="1751095564101" MODIFIED="1752396855240" MAX_WIDTH="20 cm"/>
</node>
</node>
...
<node TEXT="在GPU上创建内存来存储顶点数据，配置OpenGL解释该内存的方式，并指定如何将数据发送到显卡。然后，顶点着色器会处理我们从其内存中告诉它的顶点数量。" ID="ID_328045849" CREATED="1752298941652" MODIFIED="1752395859238"/>
</node>
</node>
